    ...
    <header className="sticky top-0 z-10 bg-gradient-to-r from-brand-800 via-brand-700 to-brand-600 shadow-md">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between overflow-x-auto">
        <div className="flex items-center gap-6">
          <span className="font-bold text-white whitespace-nowrap text-lg tracking-tight">🏥 HY-ENT Workspace</span>
          <nav className="flex items-center gap-1 text-sm whitespace-nowrap">
            {tabs.map((t) => (
              <Link
                ...
                className={
                  pathname === t.href
                    ? "flex items-center gap-1 px-3 py-1.5 rounded-full bg-white text-brand-700 font-semibold shadow-sm"
                    : "flex items-center gap-1 px-3 py-1.5 rounded-full text-blue-100 hover:bg-white/10 hover:text-white transition"
                }
              >
                <span>{t.icon}</span>
                {t.label}
        ...
        <div className="flex items-center gap-3 text-sm text-blue-100 whitespace-nowrap">
          <span>
