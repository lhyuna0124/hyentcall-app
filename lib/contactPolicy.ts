...
// 단, 안내 문구 자체에는 점수나 "역량이 낮다"는 식의 표현을 드러내지 않습니다.
export function suggestContact(
  ...
  if (risk === "MEDIUM") {
    if (comp <= 2) {
      return {
        method: "전화 노티 권장",
        detail: "시간대와 무관하게 전화로 노티하며 진행 상황을 함께 점검하는 것을 권장합니다.",
      };
    }
    if (comp === 3) {
      return {
        ...
        detail: isLateNight
          ? "새벽엔 정리된 노티장을 먼저 카톡으로 보내고 10~15분 내 회신이 없으면 전화로 전환하세요."
          : "주간엔 전화로 노티하는 것을 권장합니다.",
      };
    }
    return {
      method: isLateNight ? "카카오톡 노티 후 회신 대기" : "카카오톡 또는 전화 (선택)",
      detail: "새벽에는 정리된 노티장을 카톡으로 보내고 15~20분 내 회신이 없을 때만 전화하는 방식도 무방합니다.",
    };
  }
  ...
  if (comp <= 2) {
    return {
      ...
      detail: "낮은 위험도라도 카톡 노티 후 15분 내 회신이 없으면 전화로 한 번 확인하는 것을 권장합니다.",
    };
  }
  if (comp === 3) {
    return {
  ...
  return {
    ...
    detail: "카톡으로 노티만 남기고 통상적인 절차대로 진행 후 사후 보고로도 충분합니다. 다만 예상과 다른 소견이 새로 확인되면 위험도와 무관하게 즉시 전화하세요.",
  };
}
